...
The mixer has no sources, so Dropsift does not monitor the microphone or add playback. Its connection exists to make VoiceProcessingIO a complete, rendered duplex graph.
...
Transcript-level echo suppression remains a fallback for unsupported devices, not the primary fix. Dropsift already has the clean far-end track and aligned timestamps. Mark a mic segment as echo only when it overlaps a system segment in time and has high fuzzy token similarity; preserve segments with substantial unique mic words so local interruptions and double-talk survive.
...
- `Sources/dropsift/Audio/MicRecorder.swift` — enables duplex voice processing, reads the 9-channel format, converts it to mono, and writes the mic track.
- `Sources/dropsift/Config.swift` — currently makes voice processing default-on.
- `README.md` — currently promises echo cancellation as the default behavior.
- `Sources/dropsift/Transcription/TranscriptionCoordinator.swift` — appropriate location for cross-track transcript echo suppression.

**Flow:**

- `Sources/dropsift/RecordingSession.swift` — starts the system recorder before the mic recorder and records per-track timing offsets.
- `Sources/dropsift/Audio/SystemAudioRecorder.swift` — captures the clean far-end reference through an independent Core Audio process tap.
- `Sources/dropsift/Transcription/ParakeetEngine.swift` — transcribes each readable audio file.
- `Sources/dropsift/Transcription/TranscriptionCoordinator.swift` — shifts and merges mic and system segments.

**Downstream:**

- `Sources/dropsift/Transcription/TranscriptionEngine.swift` — defines timed transcript segments and speaker attribution.
